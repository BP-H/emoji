# -------------------------------------------------------------------------------
# The Emoji Engine — MetaKarma Hub Ultimate Fusion v5.39.0 PERFECTED OMNIVERSE
#
...
class ForkUniversePayload(TypedDict):
    ...
    """
    event: EventTypeLiteral
    company: str  # Username of the company forking (must be 'company' species)
...
# --- Configuration (Ultimate Synthesis: All parameters from all versions, with validation lambdas and expanded docs) ---
class Config:
    """
    System-wide configuration class, controlled by governance proposals.
    Aggregates all parameters from all prior versions, using the highest thresholds and most comprehensive features.
    Parameters can be updated via UPDATE_CONFIG events after governance approval.
    Each key has a validation lambda to ensure safe updates, preventing invalid states.
    This class is thread-safe with an internal lock for concurrent access.
    """
    _lock = threading.RLock()  # Internal lock for thread-safe reads/writes to config values
    VERSION = "EmojiEngine Ultimate Fusion v5.36.0 PERFECTED OMNIVERSE"  # Version string for logging and identification

    # Root Coin and Value Parameters (from all versions, with max safety thresholds)
    ROOT_COIN_INITIAL_VALUE = Decimal('1000000')  # Fixed initial value for every new user's root coin (conserved system-wide)
    FRACTIONAL_COIN_MIN_VALUE = Decimal('10')  # Minimum allowable value for any fractional coin to prevent dust
    MAX_FRACTION_START = Decimal('0.05')  # Maximum initial fraction of root coin that can be minted at once

    # Decay Parameters (encourages activity, from v5.35.0 and enhanced)
    DAILY_DECAY = Decimal('0.99')  # Daily multiplicative decay factor for karma and genesis bonuses (0.99 = 1% daily loss)

    # Share Splits for Minting (balanced splits from ultimate synthesis, ensuring 100% allocation)
    TREASURY_SHARE = Decimal('0.3333')  # Portion of minted value going to system treasury for governance funding
    REACTOR_SHARE = Decimal('0.3333')  # Portion escrowed for future reaction rewards
    CREATOR_SHARE = Decimal('0.3333')  # Portion to the creator (further split in remixes)
    REMIX_CREATOR_SHARE = Decimal('0.5')  # Sub-share to remix creator within CREATOR_SHARE
    REMIX_ORIGINAL_OWNER_SHARE = Decimal('0.5')  # Sub-share to original owner within CREATOR_SHARE
    INFLUENCER_REWARD_SHARE = Decimal('0.10')  # Small share deducted for influencers in reference chains (conserved)

    # Fees (low fees for usability, with burn for deflation)
    MARKET_FEE = Decimal('0.01')  # 1% fee on marketplace buys, going to treasury
    BURN_FEE = Decimal('0.001')  # 0.1% burn fee on transactions for deflationary pressure

    # Rate Limits and Thresholds (high limits from v5.35.0, to prevent abuse)
    MAX_MINTS_PER_DAY = 5  # Maximum mints per user per day to prevent spam
    MAX_REACTS_PER_MINUTE = 30  # Maximum reactions per user per minute for rate limiting
    MIN_IMPROVEMENT_LEN = 15  # Minimum character length for remix improvement descriptions
    MAX_PROPOSALS_PER_DAY = 3  # Maximum governance proposals per user per day
    MAX_INPUT_LENGTH = 10000  # Maximum length for user inputs (content, messages) to prevent DoS
    MAX_KARMA = Decimal('999999999')  # Absolute cap on user karma to prevent integer overflows or exploits
    MAX_REACTION_COST_CAP = Decimal('500')  # Cap on escrow release per single reaction to bound rewards

    # Karma Economics (detailed from all versions, with genesis decay over years)
    KARMA_MINT_THRESHOLD = Decimal('100000')  # Minimum karma required for non-genesis users to mint
    KARMA_MINT_UNLOCK_RATIO = Decimal('0.02')  # Karma cost per unit value minted (gating mechanism)
    GENESIS_KARMA_BONUS = Decimal('50000')  # Base bonus karma for genesis users
    GENESIS_BONUS_DECAY_YEARS = 3  # Number of years over which genesis bonus linearly decays to zero
    GENESIS_BONUS_MULTIPLIER = Decimal('10')  # Initial multiplier for genesis karma bonuses
    KARMA_PER_REACTION = Decimal('100')  # Base karma awarded per reaction to reactor
    REACTOR_KARMA_PER_REACT = Decimal('100')  # Alias for KARMA_PER_REACTION (consistency)
    CREATOR_KARMA_PER_REACT = Decimal('50')  # Karma awarded to coin creator per reaction
    STAKING_BOOST_RATIO = Decimal('1.5')  # Multiplier for staked karma in voting weight

    # Governance Parameters (tri-species harmony from ultimate, with high thresholds)
    GOV_SUPERMAJORITY_THRESHOLD = Decimal('0.90')  # 90% supermajority required for proposal approval
    GOV_QUORUM_THRESHOLD = Decimal('0.50')  # 50% quorum of total effective karma required for validity
    GOV_EXECUTION_TIMELOCK_SEC = 3600 * 24 * 2  # 48-hour timelock before executing approved proposals
    PROPOSAL_VOTE_DURATION_HOURS = 72  # 3-day voting window for proposals

    # Emoji Weights (Expanded set from all versions, with weights for reward scaling)
    EMOJI_WEIGHTS = {
    ...
    }  # Weights scale karma and escrow release; higher = more impact

    # Vaccine Patterns (Full set from all versions, categorized by severity)
    VAX_PATTERNS = {
    ...
    }  # Regex patterns for content blocking, case-insensitive
    VAX_FUZZY_THRESHOLD = 2  # Maximum Levenshtein distance for fuzzy keyword matching

    # Species and Snapshot Parameters
    SPECIES = ["human", "ai", "company"]  # Defined species for balanced governance voting
    SNAPSHOT_INTERVAL = 1000  # Number of events between automatic state snapshots

    # Allowed Policy Keys with Validation Lambdas (Comprehensive list covering all configurable params)
    ALLOWED_POLICY_KEYS = {
        "ROOT_COIN_INITIAL_VALUE": lambda v: Decimal(v) > 0,  # Must be positive
        "DAILY_DECAY": lambda v: 0 < Decimal(v) < 1,  # Between 0 and 1 exclusive
        "TREASURY_SHARE": lambda v: 0 <= Decimal(v) <= 1,  # Percentage range
        "REACTOR_SHARE": lambda v: 0 <= Decimal(v) <= 1,
        "CREATOR_SHARE": lambda v: 0 <= Decimal(v) <= 1,
        "INFLUENCER_REWARD_SHARE": lambda v: 0 <= Decimal(v) <= 0.2,  # Capped at 20%
        "MARKET_FEE": lambda v: 0 <= Decimal(v) <= 0.05,  # Capped at 5%
        "BURN_FEE": lambda v: 0 <= Decimal(v) <= 0.01,  # Capped at 1%
        "MAX_MINTS_PER_DAY": lambda v: int(v) > 0,  # Positive integer
        "MAX_REACTS_PER_MINUTE": lambda v: int(v) > 0,
        "MIN_IMPROVEMENT_LEN": lambda v: int(v) > 0,
        "GOV_SUPERMAJORITY_THRESHOLD": lambda v: 0.5 <= Decimal(v) <= 1,  # At least 50%
        "GOV_QUORUM_THRESHOLD": lambda v: 0 <= Decimal(v) <= 1,
        "GOV_EXECUTION_TIMELOCK_SEC": lambda v: int(v) >= 0,  # Non-negative
        ...
        "GENESIS_BONUS_MULTIPLIER": lambda v: Decimal(v) >= 1,  # At least 1x
    ...
    def update_policy(cls, key: str, value: Any):
        ...
        with cls._lock:
            if key not in cls.ALLOWED_POLICY_KEYS:
                raise InvalidInputError(f"Invalid policy key: {key}. Allowed keys are {list(cls.ALLOWED_POLICY_KEYS.keys())}")
            ...
            try:
                ...
                setattr(cls, key, value if isinstance(value, Decimal) or isinstance(value, int) else Decimal(str(value)))
...
def calculate_genesis_bonus_decay(join_time: datetime.datetime) -> Decimal:
    """
    ...
    if years_passed >= Config.GENESIS_BONUS_DECAY_YEARS:
        return Decimal('0')
    return Decimal('1') - Decimal(years_passed) / Config.GENESIS_BONUS_DECAY_YEARS
...
class ImprovementRequiredError(MetaKarmaError):
    ...
    def __init__(self):
        super().__init__("Remix requires a meaningful improvement description (min length {}).".format(Config.MIN_IMPROVEMENT_LEN))
...
class Vaccine:
        ...
        with self.lock:
            ...
            for word in words:
                ...
                for keyword in self.fuzzy_keywords:
                    ...
                    if dist <= Config.VAX_FUZZY_THRESHOLD:
    ...
    def _log_block(self, level: str, pattern: str, text: str):
        ...
        snippet = sanitize_text(text[:80]) + '...' if len(text) > 80 else sanitize_text(text)
...
class LogChain:
    ...
    def replay_events(self, apply_event_callback: Callable[[Dict[str, Any]], None]):
        """
        Replay all logged events in order, decoding and passing to the callback.
...
class InMemoryStorage(AbstractStorage):
    ...
    def get_user(self, name: str) -> Optional[Dict]:
        with self._users_lock:
            # Return direct reference; convention: do not modify returned dict
...
# --- Data Models (Ultimate: All fields and methods from versions, with serialization and locks) ---
class User:
    """
    User data model encompassing all features from all versions.
    Includes genesis status, species, karma (free and staked), activity tracking, consent, and owned coins.
    Thread-safe with internal lock.
    Supports effective karma calculation with staking boost and consent revocation.
    Serialization via to_dict/from_dict for snapshots and storage.
    """
    def __init__(self, name: str, genesis: bool = False, species: Literal["human", "ai", "company"] = "human"):
        ...
        if genesis:
            self._apply_genesis_bonus()

    def _apply_genesis_bonus(self):
        """Apply initial genesis bonus with decay factor."""
        decay_factor = calculate_genesis_bonus_decay(self.join_time)
    ...
    def effective_karma(self) -> Decimal:
        ...
        with self.lock:
            decay_factor = calculate_genesis_bonus_decay(self.join_time) if self.is_genesis else Decimal('1')
            return (self.karma + self.staked_karma * Config.STAKING_BOOST_RATIO) * decay_factor
    ...
    def check_rate_limit(self, action: str) -> bool:
        ...
        with self.lock:
            ...
            if action == "mint":
                if self.daily_mints >= Config.MAX_MINTS_PER_DAY:
                    return False
                self.daily_mints += 1
            elif action == "react":
                ...
                if len(recent) > Config.MAX_REACTS_PER_MINUTE:
                    return False
                ...
                if len(recent) > Config.MAX_PROPOSALS_PER_DAY:
                    return False
            return True
...
class Coin:
    ...
    def __init__(self, coin_id: str, creator: str, owner: str, value: Decimal, is_root: bool = False, universe_id: str = "main", **kwargs):
        ...
        self.content = sanitize_text(kwargs.get('content', ""))
    ...
    def release_escrow(self, amount: Decimal) -> Decimal:
        ...
        with self.lock:
            release = min(amount, self.reactor_escrow, Config.MAX_REACTION_COST_CAP)
...
class Proposal:
    ...
    def tally_votes(self, agent: 'RemixAgent') -> Dict[str, Decimal]:
        """
        ...
        total_karma_in_system = Decimal('0')
        for name in agent.storage.users.keys():
            u_data = agent.storage.get_user(name)
            if u_data and u_data["consent"]:
                karma = safe_decimal(u_data["karma"])
                staked = safe_decimal(u_data["staked_karma"])
                is_genesis = u_data["is_genesis"]
                join_time = datetime.datetime.fromisoformat(u_data["join_time"])
                decay = calculate_genesis_bonus_decay(join_time) if is_genesis else Decimal('1')
                eff_karma = (karma + staked * Config.STAKING_BOOST_RATIO) * decay
                total_karma_in_system += eff_karma
        for voter, vote in self.votes.items():
            u_data = agent.storage.get_user(voter)
            if u_data and u_data["consent"]:
                karma = safe_decimal(u_data["karma"])
                staked = safe_decimal(u_data["staked_karma"])
                is_genesis = u_data["is_genesis"]
...
class RemixAgent:
    ...
    def load_state(self):
        """
        Load system state from snapshot if exists, then replay all log events to ensure consistency.
    ...
    def save_snapshot(self):
        ...
        Includes treasury, users, coins, proposals, listings.
        Uses temp file for atomic write.
        """
        with self.lock:
            snapshot = {
                "treasury": str(self.treasury),
                "users": [self.storage.get_user(name) for name in self.storage.users],
                "coins": [self.storage.get_coin(coin_id) for coin_id in self.storage.coins],
                "proposals": [self.storage.get_proposal(p_id) for p_id in self.storage.proposals],
                "marketplace_listings": [self.storage.get_marketplace_listing(l_id) for l_id in self.storage.marketplace_listings],
                "timestamp": ts(),
            }
    ...
    def _process_event(self, event: Dict[str, Any]):
        ...
        if nonce and nonce in self.processed_nonces:
    ...
    def _apply_add_user(self, event: AddUserPayload):
        ...
        if self.storage.get_user(username):
            ...
            return
        user = User(username, event["is_genesis"], event["species"])
        user.root_coin_id = uuid.uuid4().hex
        root_coin = Coin(user.root_coin_id, username, username, Config.ROOT_COIN_INITIAL_VALUE, is_root=True, universe_id=self.universe_id)
    ...
    def _apply_mint(self, event: MintPayload):
        ...
        root_coin = Coin.from_dict(root_coin_dict)
        ...
        if not user.is_genesis and user.effective_karma() < Config.KARMA_MINT_THRESHOLD:
            raise KarmaError("Insufficient karma for mint.")
        if event["is_remix"] and len(event["improvement"]) < Config.MIN_IMPROVEMENT_LEN:
        ...
        with acquire_locks([user.lock, root_coin.lock]):
            ...
            reactor_escrow = remaining_value * Config.REACTOR_SHARE
            if event["is_remix"] and event["references"]:
                orig_coin_dict = self.storage.get_coin(event["references"][0]["coin_id"])
                if orig_coin_dict:
                    orig_coin = Coin.from_dict(orig_coin_dict)
                    orig_owner_dict = self.storage.get_user(orig_coin.owner)
                    orig_owner = User.from_dict(orig_owner_dict)
                    orig_root_dict = self.storage.get_coin(orig_owner.root_coin_id)
                    orig_root = Coin.from_dict(orig_root_dict)
                    with acquire_locks([orig_coin.lock, orig_owner.lock, orig_root.lock]):
                        orig_root.value += new_coin_value * Config.REMIX_ORIGINAL_OWNER_SHARE
            ...
            new_coin = Coin(
                new_coin_id, user_name, user_name, new_coin_value, is_root=False, universe_id=self.universe_id,
    ...
    def _apply_react(self, event: ReactPayload):
        ...
        reactor = User.from_dict(reactor_dict)
        ...
        coin = Coin.from_dict(coin_dict)
        if not is_valid_emoji(event["emoji"]):
        ...
        with acquire_locks([reactor.lock, coin.lock]):
            ...
            if creator_dict:
                creator = User.from_dict(creator_dict)
                with creator.lock:
            ...
            if release_amount > 0:
                ...
                reactor_root = Coin.from_dict(reactor_root_dict)
    ...
    def _apply_marketplace_buy(self, event: MarketplaceBuyPayload):
        ...
        buyer_root = Coin.from_dict(buyer_root_dict)
        with acquire_locks([buyer.lock, seller.lock, coin.lock, buyer_root.lock]):
            ...
            seller_root = Coin.from_dict(seller_root_dict)
    ...
    def _apply_fork_universe(self, event: ForkUniversePayload):
        ...
        Only allowed for 'company' species.
        """
        company = event["company"]
        user_dict = self.storage.get_user(company)
        if not user_dict or user_dict["species"] != "company":
            raise ForkError("Only companies can fork universes.")
        fork_id = event["fork_id"]
        fork_snapshot = f"snapshot_{fork_id}.json"
        fork_log = f"logchain_{fork_id}.log"
        fork_agent = RemixAgent(snapshot_file=fork_snapshot, logchain_file=fork_log, parent=self, universe_id=fork_id)
        # Apply custom config
        for key, value in event["custom_config"].items():
            Config.update_policy(key, value)
        self.sub_universes[fork_id] = fork_agent
        logging.info(f"Forked new universe {fork_id} by {company}")
    ...
    def _apply_cross_remix(self, event: CrossRemixPayload):
        ...
        target_agent = None
        if reference_universe in self.sub_universes:
            target_agent = self.sub_universes[reference_universe]
        elif self.parent and reference_universe == self.parent.universe_id:
            target_agent = self.parent
        if not target_agent:
        ...
        with acquire_locks([user.lock, root_coin.lock]):
            ...
            new_coin_value = value * Config.CREATOR_SHARE
            new_coin = Coin(
                coin_id, user_name, user_name, new_coin_value, is_root=False, universe_id=self.universe_id,
                is_remix=True, references=references, improvement=improvement,
                fractional_pct=str(value / Config.ROOT_COIN_INITIAL_VALUE), ancestors=[], content=""
            )
    ...
    # Additional _apply_ methods for remaining events (stubbed for completeness, expand as needed)
    ...
    def _apply_execute_proposal(self, event: Dict[str, Any]):
        ...
        if proposal_dict:
            ...
            if proposal.is_ready_for_execution():
                # Execute payload, e.g., Config.update_policy(proposal.target, proposal.payload["value"])
                proposal.status = "executed"
                self.storage.set_proposal(proposal_id, proposal.to_dict())

    # ... Implement remaining _apply_ methods similarly ...
...
class MetaKarmaCLI(cmd.Cmd):
    ...
    """
    intro = f"🚀 Welcome to {Config.VERSION} CLI\nType help or ? for available commands."
    prompt = '(EmojiEngine) > '
    ...
    def do_add_user(self, arg):
        ...
        species = parts[-1] if len(parts) > 1 and parts[-1] in Config.SPECIES else "human"
    ...
    def do_status(self, arg):
        """Show system status"""
        print(f"Users: {len(self.agent.storage.users)} | Coins: {len(self.agent.storage.coins)} | Treasury: {self.agent.treasury}")
    ...
    # Add more do_ methods for all commands
...
class TestEmojiEngine(unittest.TestCase):
    ...
    def test_mint(self):
        ...
        self.assertEqual(new_coin["value"], str(Decimal('100') * Config.CREATOR_SHARE))
    ...
    def test_react(self):
        ...
        self.agent._process_event(add_event)
        add_creator_event = AddUserPayload(
            event="ADD_USER",
            user="creator",
            is_genesis=True,
            species="human",
        ...
        self.agent._process_event(add_creator_event)
        creator_root = self.agent.storage.get_user("creator")["root_coin_id"]
        mint_event = MintPayload(
            event="MINT",
            user="creator",
            coin_id=uuid.uuid4().hex,
            value="100",
            root_coin_id=creator_root,
            references=[],
            improvement="",
            fractional_pct="0.0001",
            ancestors=[],
        ...
        self.agent._process_event(mint_event)
        react_event = ReactPayload(
            event="REACT",
            reactor="reactor",
            coin_id=mint_event["coin_id"],
            emoji="👍",
            message="Nice!",
            timestamp=ts(),
            karma_earned="0",
        ...
        self.agent._process_event(react_event)
        reactor_user = self.agent.storage.get_user("reactor")
        self.assertGreater(safe_decimal(reactor_user["karma"]), Decimal('0'))

    def test_marketplace(self):
        add_seller = AddUserPayload(
            event="ADD_USER",
            user="seller",
            is_genesis=True,
            species="human",
        ...
        self.agent._process_event(add_seller)
        seller_root = self.agent.storage.get_user("seller")["root_coin_id"]
        mint_event = MintPayload(
            event="MINT",
            user="seller",
            coin_id=uuid.uuid4().hex,
            value="100",
            root_coin_id=seller_root,
            references=[],
            improvement="",
            fractional_pct="0.0001",
            ancestors=[],
        ...
        self.agent._process_event(list_event)
        listing = self.agent.storage.get_marketplace_listing(list_event["listing_id"])
        self.assertIsNotNone(listing)
        add_buyer = AddUserPayload(
            event="ADD_USER",
            user="buyer",
            is_genesis=False,
            species="human",
        ...
        self.agent._process_event(add_buyer)
        buy_event = MarketplaceBuyPayload(
            event="MARKETPLACE_BUY",
            listing_id=list_event["listing_id"],
            buyer="buyer",
            timestamp=ts(),
            total_cost="51",  # price + fee
            nonce=uuid.uuid4().hex
        )
        self.agent._process_event(buy_event)
        coin = self.agent.storage.get_coin(mint_event["coin_id"])
        self.assertEqual(coin["owner"], "buyer")
        cancel_event = MarketplaceCancelPayload(
            event="MARKETPLACE_CANCEL",
            listing_id=uuid.uuid4().hex,  # Non-existent for test
            user="seller",
            timestamp=ts(),
            nonce=uuid.uuid4().hex
        )
        self.agent._process_event(cancel_event)  # Should skip

    def test_governance(self):
        add_creator = AddUserPayload(
            event="ADD_USER",
            user="prop_creator",
            is_genesis=True,
            species="human",
        ...
        self.agent._process_event(add_creator)
        prop_event = ProposalPayload(
        ...
        self.agent._process_event(prop_event)
        prop = Proposal.from_dict(self.agent.storage.get_proposal(prop_event["proposal_id"]))
        self.assertEqual(prop.status, "open")
        add_voter = AddUserPayload(
            event="ADD_USER",
            user="voter",
            is_genesis=True,
            species="human",
        ...
        self.agent._process_event(add_voter)
        vote_event = VoteProposalPayload(
            event="VOTE_PROPOSAL",
            proposal_id=prop_event["proposal_id"],
            voter="voter",
            vote="yes",
            timestamp=ts(),
            voter_karma="100",
        ...
        self.agent._process_event(vote_event)
        # Assume approved for test
        prop.votes["prop_creator"] = "yes"
        if prop.is_approved(self.agent):
            prop.status = "approved"
            prop.schedule_execution()
            exec_event = {"event": "EXECUTE_PROPOSAL", "proposal_id": prop.proposal_id}
            self.agent._process_event(exec_event)
    ...
    def test_staking(self):
        ...
        self.agent._process_event(add_user)
        stake_event = StakeKarmaPayload(
            event="STAKE_KARMA",
            user="staker",
            amount="500",
            timestamp=ts(),
        ...
        self.agent._process_event(stake_event)
        user_dict = self.agent.storage.get_user("staker")
        self.assertEqual(user_dict["staked_karma"], "500")
        unstake_event = UnstakeKarmaPayload(
            event="UNSTAKE_KARMA",
            user="staker",
            amount="200",
            timestamp=ts(),
        ...
        self.agent._process_event(unstake_event)
        user_dict = self.agent.storage.get_user("staker")
        self.assertEqual(user_dict["staked_karma"], "300")
