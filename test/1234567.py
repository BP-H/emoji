# -------------------------------------------------------------------------------
# The Emoji Engine — MetaKarma Hub v5.34.0 MASTERPIECE SYNTHESIS
#
...
Economic Model Highlights (Masterpiece Synthesis):
...
# --- Event Types (Expanded for maximum granularity) ---
...
class EventType:
    """Enum-like class for all event types in the system."""
...
class AddUserPayload(TypedDict):
    event: EventTypeLiteral; user: str; is_genesis: bool; species: Literal["human", "ai", "company"]; karma: str
    join_time: str; last_active: str; root_coin_id: str; consent: bool; nonce: str

class MintPayload(TypedDict):
    event: EventTypeLiteral; user: str; coin_id: str; value: str; root_coin_id: str; references: List[Dict[str, Any]]
    improvement: str; fractional_pct: str; ancestors: List[str]; timestamp: str; is_remix: bool; content: str
    genesis_creator: Optional[str]; karma_spent: str; nonce: str

class ReactPayload(TypedDict):
    event: EventTypeLiteral; reactor: str; coin_id: str; emoji: str; message: str; timestamp: str
    karma_earned: str; nonce: str

class AdjustKarmaPayload(TypedDict):
    event: EventTypeLiteral; user: str; change: str; timestamp: str; reason: str; nonce: str

class MarketplaceListPayload(TypedDict):
    event: EventTypeLiteral; listing_id: str; coin_id: str; seller: str; price: str; timestamp: str; nonce: str

class MarketplaceBuyPayload(TypedDict):
    event: EventTypeLiteral; listing_id: str; buyer: str; timestamp: str; total_cost: str; nonce: str

class MarketplaceCancelPayload(TypedDict):
    event: EventTypeLiteral; listing_id: str; user: str; timestamp: str; nonce: str
    
class ProposalPayload(TypedDict):
    event: EventTypeLiteral; proposal_id: str; creator: str; description: str; target: str
    payload: Dict[str, Any]; timestamp: str; nonce: str
    
class VoteProposalPayload(TypedDict):
    event: EventTypeLiteral; proposal_id: str; voter: str; vote: Literal["yes", "no"]; timestamp: str
    voter_karma: str; nonce: str

class StakeKarmaPayload(TypedDict):
    event: EventTypeLiteral; user: str; amount: str; timestamp: str; nonce: str

# --- Configuration (Synthesized from all versions) ---
class Config:
    """System-wide configuration parameters, intended to be controlled by governance."""
    _lock = threading.RLock()
    VERSION = "EmojiEngine UltimateMegaAgent v5.34.0 Masterpiece Synthesis"

    ROOT_COIN_INITIAL_VALUE = Decimal('1000000')
    DAILY_DECAY = Decimal('0.99')
    INFLUENCER_REWARD_SHARE = Decimal('0.05') # 5% of mint value goes to referenced creators
    ...
    KARMA_MINT_THRESHOLD = Decimal('5000') # Minimum karma to be able to mint at all
    ...
    # Karma economics
    KARMA_MINT_UNLOCK_RATIO = Decimal('0.02') # Karma needed per unit of value minted
    GENESIS_KARMA_BONUS = Decimal('50000')
    GENESIS_BONUS_DECAY_YEARS = 3
    GENESIS_BONUS_MULTIPLIER = Decimal('10')
    STAKING_BOOST_RATIO = Decimal('1.5') # Effective karma boost for staked karma
    REACTOR_KARMA_PER_REACT = Decimal('100')
    CREATOR_KARMA_PER_REACT = Decimal('50')

    # Governance
    GOV_SUPERMAJORITY_THRESHOLD = Decimal('0.66') # 2/3 majority
    GOV_QUORUM_THRESHOLD = Decimal('0.10') # 10% of total effective karma must participate
    ...
    ALLOWED_POLICY_KEYS = {
        ...
        "GENESIS_BONUS_MULTIPLIER", "STAKING_BOOST_RATIO",
    }
...
def ts() -> str: return now_utc().isoformat(timespec='microseconds') + 'Z'
...
def safe_decimal(v: Any, d=Decimal('0')) -> Decimal:
    try: return Decimal(str(v)).normalize()
    except (InvalidOperation, ValueError, TypeError): return d
def is_valid_username(n: str) -> bool: return bool(re.fullmatch(r'[A-Za-z0-9_]{3,30}', n)) if isinstance(n, str) else False
def is_valid_emoji(e: str) -> bool: return e in Config.EMOJI_WEIGHTS
...
def acquire_locks(locks: List[threading.RLock]):
    ...
    try:
        for lock in sorted_locks: lock.acquire()
        yield
    finally:
        for lock in reversed(sorted_locks): lock.release()
...
# --- Content Vaccine (with Fuzzy Matching) ---
class Vaccine:
    def __init__(self):
        self.lock = threading.RLock(); self.block_counts = defaultdict(int)
        self.compiled_patterns = {}; self.fuzzy_keywords = []
    ...
    def scan(self, text: str) -> bool:
        ...
        with self.lock:
            for lvl, pats in self.compiled_patterns.items():
                for pat in pats:
                    if pat.search(t): self._log_block(lvl, pat.pattern, text); return False
            ...
            for word in words:
                for keyword in self.fuzzy_keywords:
                    if len(word) > 2 and levenshtein_distance(word, keyword) <= Config.VAX_FUZZY_THRESHOLD:
                        self._log_block(f"fuzzy_{keyword}", f"dist({word},{keyword})", text); return False
        return True
...
# --- Audit Logchain (Pseudo-Encrypted & Thread-Safe) ---
class LogChain:
    def __init__(self, filename="logchain.log"):
        self.filename = filename; self.lock = threading.RLock(); self.entries = deque()
        self._write_queue = queue.Queue(); self._writer_thread = threading.Thread(target=self._writer_loop, daemon=True)
        self._writer_thread.start(); self._load()
    ...
    def add(self, event: Dict[str, Any]):
        event["timestamp"] = ts(); event.setdefault("nonce", uuid.uuid4().hex)
        ...
        with self.lock:
            ...
            self._write_queue.put(f"{encoded_event}||{new_hash}")
    ...
    def _writer_loop(self):
        while True:
            ...
            self.entries.append(entry_line) # Append to in-memory deque after successful write
    ...
    def verify(self) -> bool:
        ...
        for line in self.entries:
            ...
            if sha(prev_hash + encoded_event) != h: return False
            prev_hash = h
        return True

    def replay_events(self, apply_event_callback: Callable[[Dict[str, Any]], None]):
        for line in list(self.entries): # Replay from a copy
            try:
                encoded_event, _ = line.split("||")
                event_json = base64.b64decode(encoded_event).decode('utf-8')
                apply_event_callback(json.loads(event_json))
            except Exception as e: logging.error(f"Failed to replay event: {e} - Line: {line}")

# --- Data Models ---
class User:
    def __init__(self, name: str, genesis: bool = False, species: Literal["human", "ai", "company"] = "human"):
        self.name = name; self.is_genesis = genesis; self.species = species; self.lock = threading.RLock()
        self.karma = (Config.GENESIS_KARMA_BONUS * Config.GENESIS_BONUS_MULTIPLIER) if genesis else Decimal('0')
        self.staked_karma = Decimal('0'); self.join_time = now_utc(); self.last_active = self.join_time
        self.root_coin_id: Optional[str] = None; self.coins_owned: List[str] = []; self.consent = True
        self.daily_mints = 0; self._last_action_day: Optional[str] = None
    ...
    def effective_karma(self) -> Decimal: return self.karma + self.staked_karma * Config.STAKING_BOOST_RATIO
    def to_dict(self):
        with self.lock:
            return {
    ...
    def from_dict(cls, data):
        ...
        for key, value in data.items():
            if key in ["join_time", "last_active"]: setattr(user, key, datetime.datetime.fromisoformat(value.replace('Z', '')))
            elif key in ["karma", "staked_karma"]: setattr(user, key, safe_decimal(value))
            elif hasattr(user, key): setattr(user, key, value)
        return user

class Coin:
    def __init__(self, coin_id: str, creator: str, owner: str, value: Decimal, is_root: bool=False, **kwargs):
        self.coin_id = coin_id; self.creator = creator; self.owner = owner; self.value = value
        self.is_root = is_root; self.lock = threading.RLock(); self.created_at = ts()
        self.fractional_of = kwargs.get('fractional_of'); self.is_remix = kwargs.get('is_remix', False)
        self.fractional_pct = kwargs.get('fractional_pct', Decimal('0'))
        self.references = kwargs.get('references', []); self.improvement = kwargs.get('improvement', "")
        self.reactions: List[Dict] = []; self.reactor_escrow = Decimal('0')
        self.content = kwargs.get('content', ""); self.ancestors = kwargs.get('ancestors', [])
        self.genesis_creator = kwargs.get('genesis_creator', creator if is_root else None)

    def to_dict(self):
        with self.lock: return {k: (str(v) if isinstance(v, Decimal) else v) for k, v in self.__dict__.items() if k != 'lock'}
    @classmethod
    def from_dict(cls, data):
...
class Proposal:
    def __init__(self, proposal_id: str, creator: str, description: str, target: str, payload: dict):
        self.proposal_id = proposal_id; self.creator = creator; self.description = description
        self.target = target; self.payload = payload; self.created_at = ts(); self.votes = {}
        self.status = "open"; self.lock = threading.RLock(); self.execution_time: Optional[datetime.datetime] = None

    def is_expired(self) -> bool: return (now_utc() - datetime.datetime.fromisoformat(self.created_at.replace('Z',''))).total_seconds() > Config.PROPOSAL_VOTE_DURATION_HOURS * 3600
    def is_ready_for_execution(self) -> bool: return self.execution_time is not None and now_utc() >= self.execution_time
    def schedule_execution(self): self.execution_time = now_utc() + datetime.timedelta(seconds=Config.GOV_EXECUTION_TIMELOCK_SEC)
    ...
    def tally_votes(self, users: Dict[str, User]) -> Dict[str, Decimal]:
        ...
        if not active_species: return {"yes": Decimal('0'), "no": Decimal('0'), "quorum": Decimal('0')}
    ...
    def is_approved(self, users: Dict[str, User]) -> bool:
        ...
        if tally["quorum"] < Config.GOV_QUORUM_THRESHOLD: return False
    ...
    def to_dict(self):
        with self.lock: return {k: (v.isoformat() if isinstance(v, datetime.datetime) else v) for k, v in self.__dict__.items() if k != 'lock'}

class MarketplaceListing:
    def __init__(self, listing_id, coin_id, seller, price, timestamp):
        self.listing_id=listing_id; self.coin_id=coin_id; self.seller=seller
        self.price=price; self.timestamp=timestamp; self.lock=threading.RLock()
    def to_dict(self):
        with self.lock: return {k: (str(v) if isinstance(v, Decimal) else v) for k, v in self.__dict__.items() if k != 'lock'}
    @classmethod
    def from_dict(cls, data): return cls(**{k: safe_decimal(v) if k == 'price' else v for k,v in data.items()})


# --- Core Agent ---
class RemixAgent:
    def __init__(self, snapshot_file: str = "snapshot.json", logchain_file: str = "logchain.log"):
        self.vaccine = Vaccine(); self.logchain = LogChain(filename=logchain_file)
        self.users: Dict[str, User] = {}; self.coins: Dict[str, Coin] = {}
        self.proposals: Dict[str, Proposal] = {}; self.marketplace_listings: Dict[str, MarketplaceListing] = {}
        self.treasury = Decimal('0'); self.lock = threading.RLock(); self.snapshot_file = snapshot_file
        self.hooks = HookManager(); self.event_count = 0
        self.load_state()
            ...
            try:
                if os.path.exists(self.snapshot_file):
                    with open(self.snapshot_file, "r", encoding="utf-8") as f:
                        self._load_from_snapshot(json.load(f))
            except Exception as e:
                logging.error(f"Failed to load snapshot, will perform full replay: {e}")
                self._clear_state() # Ensure clean state before replay
    ...
    def _clear_state(self):
        self.users.clear(); self.coins.clear(); self.proposals.clear()
        self.marketplace_listings.clear(); self.treasury = Decimal('0')

    def _load_from_snapshot(self, snapshot: Dict[str, Any]):
        self._clear_state()
        for u_data in snapshot.get("users", []): self.users[u_data['name']] = User.from_dict(u_data)
        for c_data in snapshot.get("coins", []): self.coins[c_data['coin_id']] = Coin.from_dict(c_data)
    ...
    def save_snapshot(self):
        with self.lock:
            ...
            with open(tmp_file, "w", encoding="utf-8") as f: json.dump(snapshot, f, indent=2)
            os.replace(tmp_file, self.snapshot_file)
            logging.info(f"Snapshot saved to {self.snapshot_file}")

    def _process_event(self, event: Dict[str, Any]):
        try:
            if not self.vaccine.scan(json.dumps(event)): raise BlockedContentError("Event blocked by vaccine")
            self.logchain.add(event)
            self._apply_event(event)
            self.event_count += 1
            if self.event_count % Config.SNAPSHOT_INTERVAL == 0: self.save_snapshot()
    ...
    def _apply_event(self, event: Dict[str, Any]):
        ...
        else: logging.warning(f"Unknown event type: {event['event']}")

    # --- Event Handlers ---
    def _apply_add_user(self, event: AddUserPayload):
        if event['user'] in self.users: return
    ...
    def _apply_mint(self, event: MintPayload):
        user = self.users[event["user"]]; root_coin = self.coins[event["root_coin_id"]]
        mint_value = safe_decimal(event['value'])
        
        with acquire_locks([user.lock, root_coin.lock]):
            if user.effective_karma() < Config.KARMA_MINT_THRESHOLD and not user.is_genesis: raise KarmaError("Insufficient karma")
            if root_coin.value < mint_value: raise InsufficientFundsError("Insufficient root coin value")
            ...
            new_coin = Coin.from_dict(event); new_coin.value = new_coin_value
            new_coin.reactor_escrow = reactor_escrow_fund
            self.coins[new_coin.coin_id] = new_coin; user.coins_owned.append(new_coin.coin_id)
            user.daily_mints += 1; user.last_active = now_utc()
            
            # Distribute influencer rewards
            if event['references'] and influencer_payout > 0:
                # ... implementation to distribute influencer_payout among referenced creators
                pass

    def _apply_react(self, event: ReactPayload):
        reactor = self.users[event["reactor"]]; coin = self.coins[event["coin_id"]]
        with acquire_locks([reactor.lock, coin.lock]):
            ...
            if (creator := self.users.get(coin.creator)): creator.karma += Config.CREATOR_KARMA_PER_REACT * emoji_weight
    ...
    def _apply_marketplace_list(self, event: MarketplaceListPayload):
        if event['listing_id'] in self.marketplace_listings: return
    ...
    def _apply_marketplace_buy(self, event: MarketplaceBuyPayload):
        ...
        if not (listing := self.marketplace_listings.pop(listing_id, None)): return
        
        coin = self.coins[listing.coin_id]; buyer = self.users[event["buyer"]]; seller = self.users[listing.seller]
        with acquire_locks([coin.lock, buyer.lock, seller.lock]):
            ...
            coin.owner = buyer.name; buyer.coins_owned.append(coin.coin_id); seller.coins_owned.remove(coin.coin_id)
    ...
    def _apply_stake_karma(self, event: StakeKarmaPayload):
        user = self.users[event['user']]; amount = safe_decimal(event['amount'])
        with user.lock:
            if amount > user.karma: raise StakeError("Insufficient karma")
            user.karma -= amount; user.staked_karma += amount
            
    def _apply_revoke_consent(self, event):
        if (user := self.users.get(event['user'])): user.consent = False
...
class MetaKarmaCLI(cmd.Cmd):
    ...
    def __init__(self):
        super().__init__(); self.agent = RemixAgent()
    def do_quit(self, arg): self.agent.save_snapshot(); print("👋 State saved."); return True
    do_exit = do_quit

    def do_add_user(self, arg):
        """Add a new user: add_user <username> [genesis] [species: human/ai/company]"""
        parts = arg.split();
        if not parts: print("Usage: add_user <username> [genesis] [species: human/ai/company]"); return
        username=parts[0]; is_genesis = len(parts) > 1 and parts[1] == 'genesis'
