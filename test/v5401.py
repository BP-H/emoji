...
# --- User Model (Thread-safe, with rate limits, staking, and serialization) ---
class User:
    """
    User data model with karma, staking, genesis bonus, consent, and activity tracking.
    Thread-safe with RLock for concurrent access.
    Supports serialization to/from dict for storage.
    Rate limits stored with deque for efficiency.
    """
    def __init__(self, name: str, is_genesis: bool, species: Literal["human", "ai", "company"], config: Config):
        ...
        self.karma = config.GENESIS_KARMA_BONUS * config.GENESIS_BONUS_MULTIPLIER if is_genesis else Decimal('0')
        self.staked_karma = Decimal('0')
        self.join_time = now_utc()
        self.last_active = self.join_time
        self.root_coin_id: str = ""  # Set after creation
...
class Coin:
    ...
    def __init__(self, coin_id: str, creator: str, owner: str, value: Decimal, is_root: bool = False, universe_id: str = "main", **kwargs):
        ...
        self.content = sanitize_text(kwargs.get('content', ""), Config())  # Using default Config for static call, but ideally pass instance
